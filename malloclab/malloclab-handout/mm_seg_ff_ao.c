...
static inline void GET_LEVEL(int size);
...
static void *find_fit(size_t asize){
    ...
    while(lvl < BIN_LEVEL){
        ...
        for( bp = blist_head;bp; bp = GET_NEXT_FL(bp)){
            if(asize <= GET_SIZE(HDRP(bp)))break;
        }
        lvl++;
    }
    return bp;
...
int mm_init(void)
{
    if((heap_listp = mem_sbrk(2*WSIZE + BIN_LEVEL*DSIZE)) == NULL)
        return -1;
    blist_tbl = heap_listp;
    init_blist_table();
    heap_listp += BIN_LEVEL*DSIZE;
    APUT(heap_listp, PACK(WSIZE,3)); /*prologue heading*/
    APUT(heap_listp+WSIZE, PACK(0,3));
    heap_listp += WSIZE;
