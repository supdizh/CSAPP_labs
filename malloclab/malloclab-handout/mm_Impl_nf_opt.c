...
static void fPerror(const char* str,int bp);
...
#endif
    if(!GET_PREV_ALLOC(HDRP(bp)))rover = PREV_BLKP(bp);
    else rover = bp;
    //printf("rover redirected to %x\n",rover);
...
static void dump_heap(int asize){
    ...
    if(!asize){
        ...
        for(bp = heap_listp; GET_SIZE(HDRP(bp)) > 0; bp = NEXT_BLKP(bp)){
            printf("block %x:%x of realsize:%d alloc:%d prev_alloc:%d\n",bp,bp+GET_SIZE(HDRP(bp)),GET_SIZE(HDRP(bp)),
                                                                                GET_ALLOC(HDRP(bp)),GET_PREV_ALLOC(HDRP(bp)));
        }
        printf("epilogue %x:%x of realsize:%d alloc:%d prev_alloc:%d\n",bp,bp+GET_SIZE(HDRP(bp)),GET_SIZE(HDRP(bp)),
                                                                                GET_ALLOC(HDRP(bp)),GET_PREV_ALLOC(HDRP(bp)));
    }else{
        printf("\n------mem size[%d]----------\n",asize);
        for(bp = heap_listp; GET_SIZE(HDRP(bp)) > 0; bp = NEXT_BLKP(bp)){
