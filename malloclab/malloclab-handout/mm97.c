...
static inline int BSIZE_OF_LEVEL(int lvl);
...
static void *find_fit(size_t asize){
    ...
    char *bp = NULL;
    char *reserve,*next_blk;
    char *blist_head;

    int oldsize,expsize;
    int lvl = GET_LEVEL(asize);
    int lvlbound = MIN(BIN_LEVEL,lvl + EXP_LEVEL);
    ...
    expsize = BSIZE_OF_LEVEL(lvlbound-1);
#ifdef DEBUG
    printf("cannot find within<= %d,requested#%d,nlvl max%d \n",lvlbound,asize,expsize);
#endif
    while(lvl < BIN_LEVEL){
        blist_head = *(GET_BLIST_HEAD(lvl));
        for( bp = blist_head;bp; bp = GET_NEXT_FL(bp)){
            //Sure asize < GET_SIZE(HDRP(bp))

            oldsize = GET_SIZE(HDRP(bp));
            if(expsize > asize + 16 && oldsize>expsize+16){
                
                remove_blist(lvl,bp);
                PUT(HDRP(bp),PACK(expsize-asize,0));
                APUT(FTRP(bp), PACK(expsize-asize,0));
                reserve = bp;
                bp = NEXT_BLKP(bp);
                APUT(HDRP(bp), PACK(asize,0));
                APUT(FTRP(bp), PACK(asize,0));
                next_blk = NEXT_BLKP(bp);
                APUT(HDRP(next_blk), PACK(oldsize-expsize,0));
                APUT(FTRP(next_blk), PACK(oldsize-expsize,0));

                insert_blist(GET_LEVEL(expsize-asize),reserve);  
                insert_blist(GET_LEVEL(asize),bp);
                insert_blist(GET_LEVEL(oldsize-expsize),next_blk);
            }
#ifdef DEBUG
            printf("bling bling===\n");
#endif
